(function (imageproc) {
	...
	function applyBaseLayerOp(inputImage, processedImage, outputImage) {
		switch (currentBaseLayerOp) {
			// Apply blur
			case "blur":
				if ($("#blur-input").val() == "processed") inputImage = processedImage;
			...
			case "unsharp-mask-sharpen":
				if ($("#sharpen-input").val() == "processed")
					inputImage = processedImage;
				var grayscaleOnly = $("#gray-scale-only").is(":checked");
				var showHighpassOutput = $("#show-highpass-output").is(":checked");
				var radiusMode = $("#blur-customization").val();
				var amount = parseFloat($("#sharpen-amount").val());
				var threshold = parseFloat($("#sharpen-threshold").val());
				imageproc.unsharpMaskSharpen(
					inputImage,
					outputImage,
					grayscaleOnly,
					showHighpassOutput,
	...
	function applyOutlineLayerOp(inputImage, processedImage, outputImage) {
		switch (currentOutlineLayerOp) {
			...
			case "sobel":
				if ($("#sobel-input").val() == "processed") inputImage = processedImage;
